...
db.employees.aggregate([{$group:{_id:"$status"}}])
...
db.employees.updateMany({number:1005},{$set: {last_name:""}})
//38.update employees set salary = salary + 100 where number in (1005,1006)
db.employees.find({number:{$in:[1005,1006]}})
db.employees.updateMany({number:1005},{$inc:{salary:100}})
...
db.employees.update({number:1006},{$unset: {status:0, salary:0}})
db.employees.find()
